...
export const codeAgentFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    const sandboxId = await step.run("get-sanbox-id", async () => {
      const sandbox = await Sandbox.create("nextjs-code-agent-template2");
    ...
    const previousMessages = await step.run(
      ...
      async () => {
        ...
        const messages = await prisma.message.findMany({
          ...
          orderBy: {
            createdAt: "desc",
            //TODO: Chnage to asc if ai not understant the latest message
          },
          take: 5,
        });
          ...
          handler: async ({ files }, { step }) => {
            return await step?.run("readFiles", async () => {
              try {
                ...
                const filesContents = [];
                for (const file of files) {
                  const filesContent = await sandbox.files.read(file);
                  filesContents.push({ path: file, content: filesContent });
                }
                return JSON.stringify(filesContents);
              } catch (error) {
                return "Error: " + error;
              }
            });
